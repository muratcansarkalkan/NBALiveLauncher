#!/usr/bin/env python3
"""Build NBALiveLauncher only.

Place this file in the root of the NBALiveLauncher repository, beside its
.sln or .vcxproj.
...
def discover_cpp_target(explicit: str | None) -> Path:
    if explicit:
        ...
        if not target.is_file():
            raise BuildError(
                f"C++ build target does not exist: {target}"
            )

        return target

    preferred = ROOT / "NBALiveLauncher.sln"
    if preferred.is_file():
        return preferred

    solutions = sorted(
        path
        for path in ROOT.glob("*.sln")
    ...
    if len(projects) > 1:
        ...
        raise BuildError(
            "More than one C++ project was found. "
            "Select one with --cpp:\n  " + names
        )

    raise BuildError(
        "No .sln or .vcxproj was found beside build_all.py. "
        "Place the script in the NBALiveLauncher repository root "
        "or pass --cpp PATH."
    )


def read_build_pairs(
    target: Path,
) -> list[tuple[str, str]]:
    ...
    try:
        ...
        raise BuildError(
            f"Could not read build target {target}: {error}"
        )
    ...
    if target.suffix.lower() == ".sln":
        ...
        for line in content.splitlines():
            ...
            if (
                in_configurations
                and "EndGlobalSection" in line
            ):
                break

            if (
                not in_configurations
                or "=" not in line
            ):
                continue
...
def resolve_build_pair(
    ...
    configuration_pairs = [
        ...
        for pair in pairs
        if pair[0].lower()
        == requested_configuration.lower()
    ]

    if not configuration_pairs:
        available = ", ".join(
            f"{configuration}|{platform}"
            for configuration, platform in pairs
        )

        raise BuildError(
            f"Configuration '{requested_configuration}' "
            f"is unavailable. Valid solution configurations: "
            f"{available}"
        )
    ...
    if requested_platform:
        for configuration, platform in configuration_pairs:
            if (
                platform.lower()
                == requested_platform.lower()
            ):
                return configuration, platform

        available = ", ".join(
            platform
            for _, platform in configuration_pairs
        )

        raise BuildError(
            f"Platform '{requested_platform}' is unavailable for "
            f"'{requested_configuration}'. "
            f"Valid platforms: {available}"
        )
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build NBALiveLauncher."
    )

    parser.add_argument(
        "--cpp",
        help="C++ .sln or .vcxproj path",
    )
    ...
    parser.add_argument(
        "--clean",
        action="store_true",
        help="Clean NBALiveLauncher before building",
    )
...
def main() -> int:
    ...
    try:
        ...
        print("NBALiveLauncher build")
        print(f"Root:          {ROOT}")
        print(f"Target:        {cpp_target.name}")
        print(f"Configuration: {configuration}")
        print(
            f"Platform:      "
            f"{platform or '<MSBuild default>'}"
        )
        ...
        if outputs:
            ...
            print(
                "\nBuild succeeded; no new "
                ".asi/.dll/.exe files were found "
